...
-- privilege role that is subject to RLS. This migration creates `kairo_app`:
--   * NOSUPERUSER NOBYPASSRLS  -> RLS policies apply to it
--   * CRUD on all tables + sequence usage -> the app can still function
--
-- To activate in any environment, point the app's DATABASE_URL at kairo_app while
-- keeping migrations on the owner connection, e.g.:
--   DATABASE_URL="postgresql://kairo_app:KairoApp_Str0ng!2026@localhost:5433/kairo?schema=public"
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'kairo_app') THEN
    CREATE ROLE kairo_app LOGIN PASSWORD 'KairoApp_Str0ng!2026' NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE;
  END IF;
END $$;

GRANT USAGE ON SCHEMA public TO kairo_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO kairo_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO kairo_app;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT USAGE, SELECT ON SEQUENCES TO kairo_app;
